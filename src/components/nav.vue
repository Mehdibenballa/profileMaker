...
import imageProfile from "../components/icons/imageProfile.vue";
...
<template>
  <div
    class="bg-white items-center justify-between grid grid-cols-3 p-4 rounded-lg"
  >
    <Logo class="hidden lg:flex" />
    <Mobileicon class="lg:hidden" />
    <div class="flex gap-8 justify-center">
      <router-link
        to="/links"
        class="flex gap-4 items-center py-2 px-4 rounded-md hover:text-purple text-normal-gray text-md"
        :class="route.path == '/links' ? 'bg-light-purple text-purple' : ''"
        ><LinksIcon class="" />
        <span class="hidden lg:flex">links</span>
      </router-link>
      ...
      <router-link
        ...
        class="flex gap-4 items-center py-2 px-4 rounded-md hover:text-purple text-normal-gray text-md"
        :class="
          route.path == '/profileDet' ? 'bg-light-purple text-purple' : ''
        "
        ><imageProfile class="" />
        <span class="hidden lg:flex">Profile Details</span></router-link
      >
    </div>

    <router-link
      to="/preview"
      class="flex justify-center items-center ml-auto border border-purple text-purple hover:bg-light-purple w-[40px] h-[36px] md:w-[85px] rounded-lg"
      :class="route.path == '/preview' ? 'bg-light-purple' : ''"
      ><svg
        class="lg:hidden"
        width="20"
        height="20"
      ...
      <span class="hidden lg:flex">Preview</span></router-link
    >
  </div>
</template>
